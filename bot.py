...
bot.remove_command('help') # Désactive le !help par défaut pour utiliser !commandes
...
# 🎯 NOUVEAU : ID de la catégorie Valorant
...
class CloseTicketView(discord.ui.View):
    ...
    @discord.ui.button(label="Close", style=discord.ButtonStyle.danger, emoji="🔒", custom_id="btn_close_general")
    async def close_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if client:
            await channel.set_permissions(client, view_channel=False, send_messages=False, read_message_history=False, reason=f"Ticket fermé par {interaction.user}")

        await interaction.response.send_message("🔒 Ticket fermé : le client n'a plus accès à ce salon.", ephemeral=False)
        try: await channel.edit(name=f"closed-{channel.name}")
        except: pass

class ProductSelect(discord.ui.Select):
    def __init__(self):
        options = [discord.SelectOption(label=k, description="Gift Card", emoji=v["emoji"]) for k, v in PRODUCT_CONFIG.items()]
        super().__init__(placeholder="Je veux me régaler avec PinkGift", min_values=1, max_values=1, options=options, custom_id="select_product_main")
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        ticket_channel = await guild.create_text_channel(name=f"ticket-{user.name}", category=category, overwrites=overwrites, reason=f"Ouverture ticket PinkGift pour {product_chosen}")
        ...
        embed_ticket = discord.Embed(title=title_formatted, description=desc_formatted, color=discord.Color.from_rgb(rgb[0], rgb[1], rgb[2]))
        embed_ticket.set_image(url="https://media.discordapp.net/attachments/1517516946390908949/1517517071217332424/Ticket_cree.png?ex=6a369167&is=6a353fe7&hm=ce29c76d8a92020dd78c32b4ef8c7a7a41338df78ecf9455f930b9c0dcb1bd08&=&format=webp&quality=lossless")
...
class ValoView(discord.ui.View):
    ...
    async def buy_valo(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        desc_raw = texts["description"]
        description_str = "\n".join(desc_raw) if isinstance(desc_raw, list) else str(desc_raw)
        desc_formatted = description_str.format(user=user.mention, product="VALORANT POINTS")
        rgb = texts.get("color_rgb", [255, 192, 203])
...
async def on_ready():
    ...
    # 🎯 IMPORTANT : Enregistre les vues pour qu'elles remarchent après un redémarrage
...
async def send_tarifs(ctx):
    ...
    embed = discord.Embed(title=texts["title"], description=description_str, color=discord.Color.from_rgb(rgb[0], rgb[1], rgb[2]))
    embed.set_thumbnail(url="https://media.discordapp.net/attachments/1517516946390908949/1517517070894502108/Produits.png?ex=6a369167&is=6a353fe7&hm=06c63f7fb8cca01a4b847fd53b228c2442a158c7fe04c5f61c858a015c517c24&=&format=webp&quality=lossless")
    embed.set_image(url="https://media.discordapp.net/attachments/1517516946390908949/1517517070554890385/Photo_accueil.png?ex=6a369167&is=6a353fe7&hm=07fe98ebafb4108c5c5288ea0d18e1ce113aeebd25d71c4b433033e914d21e44&=&format=webp&quality=lossless")
...
async def cmd_paypal(ctx):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            f"{PAYPAL_EMOJI} **Nous n'acceptons uniquement PayPal comme moyen de paiement.**\n\n"
...
async def cmd_directory(ctx):
    ...
    embed.add_field(
        ...
        value=(
            "`!tarifs` : Génère l'embed des prix classiques avec menu déroulant.\n"
            "`!valo` : Génère l'embed spécial Valorant Points (sans stock, bouton vert).\n"
    ...
    embed.add_field(
        ...
        value=(
            ...
            "`!tempmute <@membre> <durée> <raison>` : Mute temporairement un utilisateur.\n"
            "`!finish <code_carte>` : Finalise la commande et affiche l'image de fin.\n"
    ...
    embed.add_field(
        ...
        value=(
            "**Syntaxe :** `!<nom_du_magasin> <montant>`\n"
            "Crée une commande en attente, renomme le salon et affiche l'embed.\n"
...
async def cmd_finish(ctx, *, code_carte: str):
    ...
    new_embed = discord.Embed(title=old_embed.title, description=new_description, color=discord.Color.from_rgb(46, 204, 113))

    for field in old_embed.fields:
        if field.name == "🔑 Carte Cadeau / Code":
            new_embed.add_field(name="🔑 Carte Cadeau / Code", value=f"```\n{code_carte}\n```", inline=False)
        else:
...
# Démarrage avec le token de ton fichier d'origine
token_discord = os.environ.get("TOKEN") or os.environ.get("DISCORD_TOKEN")
